...
# 新增：伪装成浏览器的HTTP请求头，解决403 Forbidden错误
request_headers = {
...
log_message(f"获取优选IP失败，状态码: {response.status_code}。这可能是因为服务器拒绝了脚本访问。")
...
try:
response = requests.get(url, headers=cf_api_headers)
if response.status_code == 200:
...
try:
response = requests.delete(url, headers=cf_api_headers)
if response.status_code == 200:
...
try:
response = requests.post(url, headers=cf_api_headers, json=data)
if response.status_code == 200:
