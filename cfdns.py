...
def get_cf_speed_test_ip(timeout: int = 10, max_retries: int = 5, limit: int = 20):
    """从 cf.vbar.fun 获取纯文本优选 IP 列表"""
    url = "https://cf.vbar.fun/"  # 更换为新的、对程序友好的 IP 源
...
def get_dns_record_ids(name: str):
    """获取指定名称的所有 A 记录 ID"""
    url = f"https://api.cloudflare.com/client/v4/zones/{CF_ZONE_ID}/dns_records"
    params = {"type": "A", "name": name, "per_page": 100}
    try:
        response = requests.get(url, headers=cf_api_headers, params=params, timeout=15)
        if response.ok:
            ...
            if payload.get("success"):
                ...
                log_message(f"获取DNS记录失败: {payload.get('errors')}")
                return []
        else:
...
def delete_dns_record(record_id: str) -> bool:
    ...
    try:
        response = requests.delete(url, headers=cf_api_headers, timeout=15)
        if response.ok:
            payload = response.json()
            if payload.get("success"):
                ...
                log_message(f"删除DNS记录失败: {payload.get('errors')}")
                return False
        else:
...
def create_dns_record(name: str, ip: str) -> str:
    """创建新的 A 类型 DNS 记录"""
    url = f"https://api.cloudflare.com/client/v4/zones/{CF_ZONE_ID}/dns_records"
    data = {
        "type": "A",
        "name": name,
        "content": ip,
        # 60 秒 TTL；如需自动TTL可改为 1
        "ttl": 60,
        "proxied": False,
    }
    try:
        response = requests.post(url, headers=cf_api_headers, json=data, timeout=20)
        if response.ok:
            payload = response.json()
            if payload.get("success"):
                ...
                log_message(f"为 {name} 创建指向 {ip} 的记录失败: {payload.get('errors')}")
